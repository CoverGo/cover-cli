...
import { getProductIdFromString } from "./productUtils.mjs"
...
export const useProductQueries = (apiContext) => {
	async function fetchProduct(productId, fetchScripts = false) {
		try {
			const { plan, type, version } = getProductIdFromString(productId)
			const result = await apiContext.fetchProduct(plan, type, version, fetchScripts)
...
export const useProductMutations = (apiContext) => {
	...
	async function createScript(
		...
		try {
			const result = await apiContext.createScript(
				type,
				name,
				inputSchema,
				outputSchema,
